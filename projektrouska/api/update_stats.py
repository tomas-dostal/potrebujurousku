...
def get_update_stats(request):
    ...
    except BaseException:  # bad request
        return JsonResponse(
            {'error': 'bad request. Optional parameters: "days_old"'}, safe=False)
    ...
    qu = '''
            ...
            group by CHECKSUM, POZNAMKA, AKTUALNOST, CHYBI_POCET, ZMENA_LINK_POCET, ODSTRANIT_POCET, CELK_ZMEN
            order by  DATE_UPDATED) where DATE_UPDATED >= trunc(sysdate)  - :old
            '''
...
def get_all_update_stats(request):
    ...
    qu = '''select * from(
            select DATE_UPDATED, CHECKSUM, POZNAMKA, AKTUALNOST, CHYBI_POCET, ZMENA_LINK_POCET, ODSTRANIT_POCET, CELK_ZMEN from info
            order by  DATE_UPDATED) where DATE_UPDATED >= (trunc(sysdate)  - :old)
            '''
    with connection.cursor() as cursor:
        cursor.execute(qu, {"old": show_last_days})
